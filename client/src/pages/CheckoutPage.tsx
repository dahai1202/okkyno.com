  ...
  const onSubmit = async (data: CheckoutFormValues) => {
    ...
    try {
      ...
      if (data.paymentMethod === 'crypto') {
        ...
        const invoiceResponse = await fetch('/api/payments/nowpayments/create-invoice', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            amount: total,
            ...
            description: `Okkyno Order #${createdOrder.id}`,
            successUrl: `${window.location.origin}/checkout/success?orderId=${createdOrder.id}`,
            cancelUrl: `${window.location.origin}/checkout?orderId=${createdOrder.id}&cancelled=true`
          })
        });
        ...
        // PayPal - show message that it's not configured yet
        toast({
          title: "PayPal Not Available",
          description: "PayPal is not configured yet. Please use another payment method.",
          variant: "destructive",
        });
        setIsSubmitting(false);
        return;
      } else {
        // Credit card / Bank transfer - mark as pending and redirect
        await clearCart();
                          ...
                          <FormItem className="space-y-3">
                            <FormControl>
                              ...
                              >
                                ...
                                <FormItem className="flex items-center space-x-3 space-y-0 bg-gradient-to-r from-orange-50 to-yellow-50 p-3 rounded-lg border border-orange-200">
                                  <FormControl>
                                    <RadioGroupItem value="crypto" />
                                  </FormControl>
                                  <FormLabel className="font-normal flex items-center gap-2">
                                    <span className="text-orange-600">₿</span>
                                    Cryptocurrency (Bitcoin, ETH, USDT & more)
                                    <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Powered by NOWPayments</span>
                                  </FormLabel>
                                </FormItem>
                                <FormItem className="flex items-center space-x-3 space-y-0">
                                  <FormControl>
